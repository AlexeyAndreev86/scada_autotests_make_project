...
def test_iec101_slave(driver):
    ...
    print(time.asctime(), '\nПараметрируем модуль IEC 60870-101 (ведомый)')
    time.sleep(0.75)

    try:
        driver.find_element(By.XPATH, "//span[contains(text(), 'IEC 60870-101 (ведомый)')]").click()
    except:
    ...
    if len(chanels)>0:
        print('удаляем все старые каналы МЭК-101 (ведомый)')
        time.sleep(1)
    ...
    driver.find_element(By.XPATH, '//div[@view_id="$text2"]//input').clear()
    driver.find_element(By.XPATH, '//div[@view_id="$text2"]//input').send_keys('3')

    sinchro = driver.find_element(By.XPATH, '//div[@view_id="$checkbox6"]//button').get_attribute('aria-checked')
    if sinchro == 'true':
        driver.find_element(By.XPATH, '//div[@view_id="$checkbox6"]//button').click()

    driver.find_element(By.XPATH, '//label[text()="Длина общего адреса ASDU:"]//following-sibling::input').clear()
    driver.find_element(By.XPATH, '//label[text()="Длина общего адреса ASDU:"]//following-sibling::input').send_keys('2'+ Keys.ENTER)  
    ...
    driver.find_element(By.XPATH, r'//div[@view_id="$suggest1"]//div[text()="M_DP_NA_1 \ DPI"]').click()
    ...
    print(time.asctime(), 'test_0010 IEC 60870-101 (ведомый) пройден')
